...
const CARREFOUR_SEARCH_ENDPOINT =
...
function chooseSeller(sellers: VtexSeller[] = []): VtexSeller | undefined {
  return [...sellers].sort((left, right) => {
    ...
    const leftAvailable = Boolean(
      leftOffer?.IsAvailable && (leftOffer.AvailableQuantity ?? 0) > 0 && (leftOffer.Price ?? 0) > 0,
    )
    const rightAvailable = Boolean(
      rightOffer?.IsAvailable && (rightOffer.AvailableQuantity ?? 0) > 0 && (rightOffer.Price ?? 0) > 0,
    )
    if (leftAvailable !== rightAvailable) return leftAvailable ? -1 : 1
    if (Boolean(left.sellerDefault) !== Boolean(right.sellerDefault)) return left.sellerDefault ? -1 : 1
    const priceDifference = (leftOffer?.Price ?? Number.MAX_SAFE_INTEGER) -
      (rightOffer?.Price ?? Number.MAX_SAFE_INTEGER)
    if (priceDifference !== 0) return priceDifference
    return (left.sellerId ?? '').localeCompare(right.sellerId ?? '')
  })[0]
}

/** Convierte cada SKU/variante VTEX en un Product consumible por @sba/core. */
...
function searchTermFor(request: CatalogRequest): string {
  const explicitPreference = request.preferences.map((value) => value.trim()).find(Boolean)
  return (explicitPreference ?? CATEGORY_SEARCH_TERMS[request.category] ?? request.category).slice(0, 120)
}
...
export class VtexCatalogAdapter implements CatalogAdapter {
  ...
  constructor(
    private readonly fallback: LocalCatalogAdapter,
    options: VtexCatalogAdapterOptions = {},
  ) {
  ...
  async getCatalog(request: CatalogRequest): Promise<CatalogResult> {
    const searchTerm = searchTermFor(request)
    ...
    try {
      ...
      if (!Array.isArray(payload)) throw new Error('respuesta VTEX inválida')

      return {
        products: normalizeVtexProducts(payload as VtexProduct[], request.category),
      ...
      console.warn(`Catálogo VTEX no disponible (${reason}); usando fallback local.`)
      return {
        ...local,
        provider: this.provider,
        source: 'local-fallback',
        label: 'Catálogo local (fallback de Carrefour / VTEX)',
        searchTerm,
